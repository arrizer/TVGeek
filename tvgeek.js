...
(function() {
  var App, Async, CLI, CLIColor, Config, File, FileSystem, HTTP, LibXML, Lock, Log, Path, Readline, SizeFormatter, TheTVDBAPI, UserPrompt, app, log, options, prompt, sprintf,
    __slice = [].slice;
  ...
  Lock = (function() {
    ...
    Lock.prototype.acquire = function(critical) {
      this.queue.push(critical);
      if (!this.locked) {
        return this.next();
      }
    };
    ...
    Lock.prototype.next = function() {
      ...
      this.locked = true;
      return this.queue.shift()((function(_this) {
        return function() {
          _this.locked = false;
          return _this.next();
        };
  ...
  UserPrompt = (function() {
    ...
    UserPrompt.prototype.pick = function(options, message, next) {
      ...
      return this.lock.acquire((function(_this) {
        return function(release) {
          ...
          _this.rl.resume();
          index = null;
          return _this.prompt('Pick one: ', function(index) {
            if (!/^\d+$/.test(index) || parseInt(index) < 1 || parseInt(index) > options.length) {
    ...
    UserPrompt.prototype.prompt = function(message, next) {
      return this.rl.question(message, (function(_this) {
        return function(index) {
          var error;
          if ((error = next(index)) != null) {
            console.log(error);
            return _this.prompt(message, next);
          }
        };
      })(this));
    };
            ...
            if (shows.length === 1) {
              ...
              return prompt.pick(shows, "To which show does " + _this.filename + " belong?", function(show) {
                if (show != null) {
                  ...
                  return next(new Error("User did not pick a show"));
                }
              });
            }
          }
        ...
        return function(error, files) {
          ...
          for (_i = 0, _len = files.length; _i < _len; _i++) {
            ...
            if (Path.basename(file, Path.extname(file)) === basename) {
              ...
              FileSystem.stat(filename, function(error, stat) {
                return FileSystem.statSync(_this.filepath, function(error, info) {
                  var overwrite;
                  _this.info = info;
                  overwrite = false;
